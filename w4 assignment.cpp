...
int piceX[10] = { 0, 426, 852, 0, 426, 852, 0,  426, 852, 426 };
int piceY[10] = { 480, 480, 480, 240, 240, 240, 0, 0, 0, 240 };
...
void mouseCallback(ObjectID object, int x, int y, MouseAction action) {
	for (int i = 0; i < 9; i++) {
		...
		if (object == startButton) {
			locateObject(pice[i], scene1, piceX[(rand() + i + 9) % 9], piceY[(rand() - i + 9) % 9]);
			showObject(pice[9]);
...
int main() {
	...
	locateObject(pice[9], scene1, piceX[9], piceY[9]);
	startGame(scene1);
}
